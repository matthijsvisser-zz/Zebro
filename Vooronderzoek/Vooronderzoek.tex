...
\textbf{Star network}\\
The star network in figure \ref{fig:STAR} uses the device marked as '1' as an central switch to reach out to other devices.\cite{starwiki} A central device is not desirable in a swarm network, so the star topology is not suitable. In a star network, the central device is the single point of faillure, which will stop the network from working if it fails.
...
\end{itemize}
Adaptive robust tree is a meshed tree routing approach to address the nodes. Each node is adaptively addressed during the tree formation. Because of this method the tree is free of single point failures (SPOF). This means that there is always a redundant path if a node fails. Each node keeps track of an ART table (ARTT), to map it's branches. In this table the adresses of the connected branches are stored.
