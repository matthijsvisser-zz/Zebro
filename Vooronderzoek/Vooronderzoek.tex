...
For the mapping the positions of all robots in the swarm a localization technique needs to be implemented. The robots can use information as
relative position and relative orientation of their neighbors to interpret and fuse the information collected by other robots. 
With relative position the units can define the distance relative to each other. When all robots in the swarm knows the distance opposite to each other a mapping of the current formation can be created. However this can result in multiple solutions because there are no orientations involved. This means that the map could be mirrored. As seen in in Fig \ref{Angle}
...
This can be seen in fig \ref{Angle} 
...
This can also be achieved by the use of stationary beacons. When a minimum of three beacons is placed on the field to be measured the location of the robots in the field can be measured, this is called triangulation. Triangulation is the process of determining the location of a point by measuring angles to it from known points. This is illustrated in fig \ref{triangulation}.
...
As seen in fig \ref{orientation} each robot has a common reference direction, the direction relative to this reference is called the relative orientation of the robot and can be visualized as shown in fig . With the known distance ‘d’, the mapping of the swarm can be completed.
